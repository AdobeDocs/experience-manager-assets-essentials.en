...
Assets Essentials also allows you to set expiration date on assets available in the repository. You can then filter the search results based on an `Expired` asset status. In addition, you can specify an expiration date range for assets to further filter your search results.
